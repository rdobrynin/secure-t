...
Если ваш бэкенд использует жёстко прописанные параметры TLS, старые VPN-шлюзы или IoT-устройства с ограниченной памятью — переход на PQC может «положить» ваш сервис даже без всякой атаки: новые ключи просто не влезают в старые пакеты.
...
Микеле Моска формализовал риск простым неравенством: **X + Y > Z**, где X — срок жизни данных (лет), Y — время на миграцию (лет, всегда дольше чем кажется), Z — время до появления CRQC. Если X + Y > Z — вы уже опоздали.
...
см файл INTERACTIVE.md и INTERCATIVE.html
== Конец интерактива ==
...
**Как быстро обнаружить.** Прямого индикатора нет. Косвенные признаки: аномальный объём исходящего трафика через транзитные узлы, неожиданные BGP-изменения, появление «зеркал» трафика в netflow-логах.
...
**ML-DSA (ранее Dilithium) — FIPS 204.** Цифровая подпись на решётках. Основной выбор для сертификатов, авторизации, TLS — быстрая проверка подписи критична, когда браузер проверяет цепочки сертификатов за миллисекунды.

**SLH-DSA (ранее SPHINCS+) — FIPS 205.** Цифровая подпись на хеш-функциях. Безопасность не зависит от решёток — если в теории решёток найдут уязвимость, SLH-DSA выстоит. Медленный, подпись весит 8–40 КБ. Применять там, где проверка редка: подпись прошивок, корневые сертификаты.
