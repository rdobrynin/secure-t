...
function checkQuantumRisk(
  dataType: string,
  shelfLifeYears: number,
  migrationEstimateYears: number,
  crqcYear: number = YEAR_CRQC_ARRIVAL
): RiskResult {
  const currentYear = new Date().getFullYear();
  const timeToCollapseZ = crqcYear - currentYear;
  const totalExposure = shelfLifeYears + migrationEstimateYears;

  console.log(`--- Анализ для: ${dataType} ---`);
  if (totalExposure > timeToCollapseZ) {
    const riskGap = totalExposure - timeToCollapseZ;
    console.log(`🔴 КРИТИЧЕСКИЙ РИСК. Вы опоздали на ${riskGap} лет.`);
    return { isSafe: false, years: riskGap };
  } else {
    const safetyMargin = timeToCollapseZ - totalExposure;
    console.log(`🟢 Безопасно. Запас: ${safetyMargin} лет.`);
    return { isSafe: true, years: safetyMargin };
  }
}

checkQuantumRisk("Session Tokens", 0, 3);
checkQuantumRisk("Genomic Data / Trade Secrets", 15, 5);
...
// Default-значения Node.js могут включать устаревшие
// алгоритмы — не полагайтесь на них.
// ────────────────────────────────────────────────

// ДО (Legacy) — уязвимая конфигурация
function createContextLegacy(): tls.TlsOptions {
  return {
...
function createContext(): tls.TlsOptions {
  return {
    key:  fs.readFileSync("server.key"),
    cert: fs.readFileSync("server.crt"),
    ...
    // Требует Node.js 22+ и OpenSSL 3.5+
    ecdhCurve: "x25519_mlkem768:x25519",
...
const server = tls.createServer(createContext(), (socket) => {
...
function createQuantumSafeContext(): tls.TlsOptions {
  return {
    key:  fs.readFileSync("server.key"),
    cert: fs.readFileSync("server.crt"),
